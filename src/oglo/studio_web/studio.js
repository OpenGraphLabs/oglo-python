...
if (!Number.isInteger(currentStep) || currentStep < 1 || currentStep > 5) currentStep = 1;
let unlockedStep = Number(sessionStorage.getItem("oglo-studio-unlocked") || 1);
if (!Number.isInteger(unlockedStep) || unlockedStep < 1 || unlockedStep > 5) unlockedStep = 1;
let shownStep = null;
...
async function discoverDevices() {
  ...
  try {
    ...
    const cameras = devices.cameras.map((camera) => ({
      value: camera.index, label: `${camera.label} · index ${camera.index}`,
    }));
    const ov = devices.cameras.filter((camera) => /ovision/i.test(camera.label));
    fillDeviceSelect("camera-index", cameras, "Select a camera", ov.length === 1 ? ov[0].index :
...
function liveReady() {
  if (!pairConnected(currentStatus) || !currentLive?.camera?.ready ||
...
function renderLive(live) {
  ...
  badge("camera-live-badge", cameraFresh,
        live.camera?.error || (cameraFresh ? `Live · ${live.camera.age_ms} ms ago` : "Waiting for frames"),
        !!live.camera?.error);
  badge("record-live-badge", liveReady(), liveReady() ? "Camera + both gloves live" : "Check device streams",
        !!live.camera?.error || Object.keys(live.errors || {}).length > 0);
  $("camera-dot").className = `status-dot${cameraFresh ? " ready" : live.camera?.error ? " bad" : ""}`;
  if (currentStatus) render(currentStatus);
}
...
function makeEpisode(episode) {
  ...
  name.textContent = episode.task_description || episode.id;
  const state = document.createElement("small");
  state.className = episode.complete ? "ok" : "bad";
  state.textContent = episode.complete ? episode.selection.toUpperCase() : "INCOMPLETE";
  head.append(name, state);
  wrapper.append(head);
  const detail = document.createElement("p");
  detail.textContent = episode.complete ?
    `${(episode.gloves || []).map((hand) => hand.side).join(" + ")} · ${episode.camera?.frames_decoded || 0} video frames` :
    (episode.error || "Recording in progress");
  wrapper.append(detail);
  if (episode.complete && episode.camera?.video) {
  ...
  if (episode.complete && episode.gloves?.length === 2) {
...
function render(status) {
  ...
  $("camera-device").textContent = status.camera ? `Index ${status.camera.index}${status.camera.error ? ` · ${status.camera.error}` : ""}` : "Not connected";
  updateConnectButton();
  ...
  if (status.state === "recording" || status.state === "finalizing" || status.state === "review") {
    unlockedStep = Math.max(unlockedStep, 4);
    if (status.state === "recording" && currentStep !== 4) currentStep = 4;
  }
  if (pair && !calibrationConfirmed && !["recording", "finalizing", "review"].includes(status.state)) {
  ...
  const needsNative = $("profile").value === "annotation_handoff";
  const cameraSupportsNative = !!status.camera?.native_device_timestamps;
  $("start").disabled = status.state !== "ready" || !liveReady() || busy || (needsNative && !cameraSupportsNative);
  $("stop").disabled = status.state !== "recording" || busy;
  $("profile-status").textContent = needsNative && !cameraSupportsNative ?
    "This camera has no native frame timestamp. Choose Source archive or connect a native camera adapter." :
    (needsNative ? "Native frame timing is available for this adapter." :
      "The source archive keeps video and both gloves with host timing.");

  const signature = JSON.stringify(status.episodes);
  if (signature !== renderedEpisodes) {
    renderedEpisodes = signature;
    const holder = $("episodes");
    holder.replaceChildren();
    if (!status.episodes.length) {
      const empty = document.createElement("p");
      empty.className = "empty";
      empty.textContent = "No takes yet. Your first recording will appear here.";
      holder.append(empty);
    } else for (const episode of [...status.episodes].reverse()) holder.append(makeEpisode(episode));
  }
  $("episode-count").textContent = `${status.episodes.length} take${status.episodes.length === 1 ? "" : "s"}`;
  ...
  $("next-4").disabled = count === 0 || busy;
  $("export-count").textContent = `${count} kept episode${count === 1 ? "" : "s"}`;
  $("export-status").textContent = count ? `${profile === "source_archive" ? "Source archive" : "Annotation handoff"} ready` : "Nothing to export for this profile";
  $("export").setAttribute("aria-disabled", String(count === 0));
  $("export").style.pointerEvents = count ? "" : "none";
  $("export").style.opacity = count ? "" : ".45";
  if (count) unlockedStep = Math.max(unlockedStep, 5);
  setStep(currentStep);
}
...
document.addEventListener("keydown", (event) => {
  ...
  if (event.repeat || event.altKey || event.ctrlKey || event.metaKey || currentStep !== 4) return;
...
$("next-4").addEventListener("click", () => { unlockedStep = 5; setStep(5); });
...
$("connect").addEventListener("click", async () => {
  ...
  await perform("/api/connect", {
    camera_index: Number($("camera-index").value),
