...
`oglo.studio_realsense.RealSenseCameraWorker` is a second native-timing reference: it sets `native_device_timestamps` true (every color frame carries real camera-clock time) but `postprocessing_capable` false, since a single-view RealSense recording is not an OVISION stereo source and cannot back the `og_center_postprocessing` profile. [`examples/camera_glove/realsense.py`](../examples/camera_glove/realsense.py) is its standalone reference.
