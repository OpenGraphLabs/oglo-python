"""Config parsing for the single firmware-0.9.10+/schema-6 contract."""
...
# Schema shape used by firmware 0.9.10 and newer.
CFG_V6 = json.loads(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({**CFG_V6, "fw_rev": "0.9.9"}, "requires firmware 0.9.10"),
...
def test_the_lexicographic_trap_specifically():
    """`"0.10.0" < "0.9.0"` as strings. Numeric comparison must not fall into it."""
    assert "0.10.0" < "0.9.0"  # the trap
    assert _fw_at_least("0.10.0", (0, 9, 10)) is True  # not fallen into
